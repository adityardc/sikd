...
class coLap_agenda_sentral_int extends Controller
{
    public function index()
    {
    	return view('lap_agenda_sentral_int');
    }
    ...
    public function list(Request $request)
    {
        ...
        $listSurat = DB::table("tbl_surat_masuk")
        				->whereRaw("((',' || RTRIM(tujuan) || ',') LIKE '%,".$id_asal.",%'
                                    ...
                                    AND jenis_surat = 0 AND tahun_surat = ".$request->tahun)
                        // ->where('jenis_surat', 0)
        ...
        foreach ($listSurat as $list) {
            ...
            $row[] = $list->perihal;
            $row[] = "<button type='button' class='btn btn-default btn-xs shiny icon-only magenta tooltip-magenta' onclick='detail(".$list->id_surat_masuk.")' data-toggle='tooltip' data-placement='top' data-original-title='Agenda Surat' href='javascript:void(0);'><i class='fa fa-eye'></i></button>";
            $data[] = $row;
        }
