...
class coLap_disposisi_dir_sk extends Controller
{
    public function index()
    {
    	$tujuan = DB::table('tbl_bagian')->where('grup_bagian', 0)->orderBy('grup_bagian')->orderBy('id_bagian')->get();
    	return view('lap_disposisi_dir_sk', compact(['tujuan']));
    }
    ...
    public function list(Request $request)
    {
        ...
        $listSurat = DB::table("tbl_agenda_direksi")
                        ...
                        ->whereRaw("((',' || RTRIM(tbl_agenda_direksi.tujuan_dispo) || ',') LIKE '%,".$id_asal.",%'
                    				OR tbl_surat_keluar.id_bagian = ".$id_asal.")
                                    AND tbl_agenda_direksi.jenis_surat = 2 AND tbl_agenda_direksi.id_tujuan_dispo = ".$request->id_direktur." AND tbl_agenda_direksi.tahun_agenda = ".$request->tahun)
