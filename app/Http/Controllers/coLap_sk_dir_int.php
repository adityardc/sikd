...
class coLap_sk_dir_int extends Controller
{
    public function index()
    {
    	$tujuan = DB::table('tbl_bagian')->where('grup_bagian', 0)->orderBy('id_bagian')->get();
    	return view('lap_sk_dir_int')->with('tujuan', $tujuan);
    }
    ...
    public function list()
    {
        ...
        foreach ($konseptor as $list) {
            ...
            $row[] = $baris;
            $row[] = $list->perihal;
            $row[] = "<button type='button' class='btn btn-default btn-xs shiny icon-only yellow tooltip-yellow' onclick='detail(".$list->id_surat_keluar.")' data-toggle='tooltip' data-placement='top' data-original-title='Detail Surat' href='javascript:void(0);'><i class='fa fa-eye'></i></button>";
            $data[] = $row;
        }
    ...
    public function detail($id)
    {
        ...
        return view('modal/modal_detailLap_sk_dir_int', compact(['detail','tindasan','tujuan','tujuan_dispo','direksi_dispo','agenda']));
    }
}
