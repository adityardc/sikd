...
class coSurat_keluar_dir_int extends Controller
{
    public function index()
    {
    	return view('surat_keluar_dir_int');
    }
    ...
    public function tambah()
    {
        ...
        return view('form_tambah/add_surat_keluar_dir_int', compact(['sifat','all','idBagian','kdBagian']));
    }
    ...
    public function list()
    {
        ...
        foreach ($konseptor as $list) {
            ...
            $row[] = $baris;
            $row[] = $list->perihal;
            $row[] = "<button type='button' class='btn btn-default btn-xs shiny icon-only danger tooltip-danger' onclick='detail(".$list->id_surat_keluar.")' data-toggle='tooltip' data-placement='top' data-original-title='Detail Surat' href='javascript:void(0);'><i class='fa fa-eye'></i></button>
                      <a href='surat_keluar_direktur_internal/".$list->id_surat_keluar."/ubah' class='btn btn-default btn-xs shiny icon-only danger tooltip-danger' data-toggle='tooltip' data-placement='top' data-original-title='Ubah Surat'><i class='fa fa-pencil'></i></a>";
            $data[] = $row;
        }
    ...
    public function ubah($id)
    {
        ...
        $url = url('surat_keluar_direktur_internal/'.$id.'/update');
        $selected_tindasan = explode(",", $surat->tindasan);
        $selected_tujuan = explode(",", $surat->tujuan);
        $sifat = DB::table('tbl_sifat_surat')->orderBy('id_sifat_surat')->get();
        $all = DB::table('tbl_bagian')->orderBy('grup_bagian')->orderBy('id_bagian')->get();
        $idBagian = DB::table('tbl_bagian')->where('id_bagian', Auth::user()->id_bagian)->first();
        return view('form_ubah/edit_surat_keluar_dir_int', compact(['surat','sifat','all','idBagian','selected_tindasan','selected_tujuan','url']));
    }
